...
/* ================================================================
   BACKLOG SYNC
================================================================ */
function updateBacklogPanel() {
  const panel = document.getElementById('backlog-sync-panel');
  const text  = document.getElementById('sync-backlog-text');
  const btn   = document.getElementById('sync-backlog-btn');
  ...
  chrome.storage.local.get(['syncQueue'], (data) => {
    ...
    const pendingCount = queue.filter(item => 
      item.status === 'pending' || (item.status === 'failed' && item.retryable)
    ).length;

    if (pendingCount > 0) {
      panel.style.display = 'flex';
      text.textContent = `Sync ${pendingCount} Backlog Solution${pendingCount > 1 ? 's' : ''}`;
      btn.disabled = false;
    } else {
...
function initBacklogSync() {
  ...
  btn.addEventListener('click', () => {
    btn.disabled = true;
    text.textContent = 'Syncing Backlog...';

    chrome.runtime.sendMessage({ type: 'FORCE_RUN_QUEUE' }, (res) => {
      // Small timeout to allow the background task to run and update storage
      setTimeout(() => {
        loadDashboard(false);
      }, 1500);
    });
  ...
  // Real-time synchronization badge updates
  chrome.storage.onChanged.addListener((changes, areaName) => {
    if (areaName === 'local' && changes.syncQueue) {
      updateBacklogPanel();
    }
  });
}
